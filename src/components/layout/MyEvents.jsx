import { LuTrendingUp, LuInfo, LuTrash2, LuLogOut } from "react-icons/lu";
      ...
      {loadingEvents ? (
        ...
        <div className="bg-white p-3 rounded-2xl flex flex-col gap-3 max-h-[225px] overflow-y-auto">
          {userEvents.map((event) => (
            <div
              key={event.id}
              className="relative flex bg-[#F6F6F6] rounded-2xl p-4"
            >
              {(() => {
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          console.error(
                            "Erro ao desconfirmar presença:",
                            error
                          );
                          alert("Erro ao desconfirmar. Tente novamente.");
                        }
                      }}
                      className="flex flex-col items-center justify-center bg-white rounded-full p-2 shadow-md hover:bg-yellow-100 transition ease-in-out"
                    >
                      <LuLogOut size={24} className="text-orange-500" />
                    </button>
        ...
        <div className="bg-white p-6 rounded-2xl text-center">
          <p className="text-gray-500">
            Você ainda não criou nenhum evento ou participou de algum
          </p>
        </div>
      )}
    </section>
