...
export default function EventDetail() {
  ...
  const isCreator =
    user && event && String(user.uid) === String(event.createdBy);
  const isParticipant =
    user &&
    event &&
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const updated = await eventService.addParticipant(id, user.displayName);
      setEvent(updated);
    } catch (err) {
  ...
  const handleUnconfirm = async () => {
    ...
    try {
      setProcessingParticipant(true);
      const updated = await eventService.removeParticipant(
        id,
        user.displayName
      );
      setEvent(updated);
    } catch (err) {
      console.error("Erro ao desconfirmar presença:", err);
      setError("Erro ao cancelar confirmação. Tente novamente.");
    } finally {
      setProcessingParticipant(false);
    }
  };
  ...
  if (error)
    return (
      <div className="p-6">
        ...
        <button
          onClick={() => navigate(-1)}
          className="mt-4 px-4 py-2 bg-[#2E8FE0] text-white rounded"
        >
          Voltar
        </button>
      </div>
        ...
        <div className="bg-white rounded-xl shadow p-6">
          ...
          <div className="mt-4">
            ...
            <div className="flex items-center justify-between mt-2">
              <div className="font-medium">
                {confirmedCount} {confirmedCount === 1 ? "pessoa" : "pessoas"}
              </div>

              {user ? (
                isCreator ? (
                  <div className="text-sm text-gray-500">Você é o criador</div>
                ) : isParticipant ? (
                  <button
                  ...
                  <button
                    ...
                    disabled={processingParticipant}
                    className="px-4 py-2 bg-[#2E8FE0] text-white rounded"
                  >
                    {processingParticipant
                      ? "Confirmando..."
                      : "Confirmar presença"}
